   ...
   <div class="container">
      <div class="row">
         <div class="col-md-12 col-xl-12">
            @include('auth.statement.sidebar')
         </div>
         <div class="col-md-12 col-xl-12">
            <div class="tab-content">
               <div class="tab-pane fade {{ empty(request()->get('sec')) ? 'show active' : ''}}" id="bets">
                  <div class="card">
                           ...
                           <table class="table table-bordered table-hover">
                              ...
                              <tbody>
                                 ...
                                 <tr>
                                    ...
                                    <td>{{ $bet->bet_rate }}</td>
                                    <td>{{ $bs->currency_symbol }} {{ number_format($bet->return_amount,2) }}</td>
                                    <td class="text-center">
                                       @if($bet->status == 'pending')
                                       <img src="{{ asset('static/images/pending.gif') }}" alt="Pending" class="img-table">
                                       @elseif($bet->status == 'win')
                                       <img src="{{ asset('static/images/win.png') }}" alt="Win" class="img-table">
                                       @elseif($bet->status == 'loss')
                                       <img src="{{ asset('static/images/loss.png') }}" alt="Loss" class="img-table">
                                       @elseif($bet->status == 'refund')
                                       <img src="{{ asset('static/images/refund.png') }}" alt="Refund" class="img-table">
                                       @endif
                                    </td>
               ...
               @include('auth.statement.deposit')
               @include('auth.statement.withdraw')
               @include('auth.statement.balanceTransfer')
               @include('auth.statement.transaction')
            </div>
         </div>
      </div>
   </div>
