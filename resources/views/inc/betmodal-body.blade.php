<div class="block p-6 rounded-lg shadow-lg bg-white">
    <div>
        ...
        <h5 class="text-gray-600 font-light p-1">{{$option->name}} <span class="bg-red-600 text-white p-1">{{$option->bet_rate}}</span></h5>
    </div>
    <form method="POST" action="{{ route('user.bet') }}">
        @csrf
            <div class="flex space-x-4 py-2">
                <label class="cursor-pointer border border-green-800 rounded-full py-1 px-4">
                    <input type="radio" class="plan_name" name="amount" id="amount" value="100"  style="display:none"checked>100
                 </label>
                <label class="cursor-pointer border border-green-800 rounded-full py-1 px-4">
                    <input type="radio" class="plan_name" name="amount" id="amount" value="500"  style="display:none">500
                 </label>
                <label class="cursor-pointer border border-green-800 rounded-full py-1 px-4">
                    <input type="radio" class="plan_name" name="amount" id="amount" value="1000" style="display:none" >1000
                 </label>
                <label class="cursor-pointer border border-green-800 rounded-full py-1 px-4">
                    <input type="radio" class="plan_name" name="amount" id="amount" value="3000" style="display:none" >3000
                 </label>
                <label class="cursor-pointer border border-green-800 rounded-full py-1 px-4">
                    <input type="radio" class="plan_name" name="amount" id="amount" value="5000" style="display:none" >5000
                 </label>
            </div>
            ...
            <div class="py-2 space-y-2">
                <div class="flex justify-between bg-emerald-600 w-full rounded-sm"
                    ...
                    <span class="text-white font-light p-1">Total Shake</span>
                    <span class="bg-emerald-800 text-white font-light p-1 px-4">$100</span>
                </div>
                <div class="flex justify-between bg-emerald-600 w-full rounded-sm "
                    data-bs-toggle="modal" data-bs-target="#betModal" data-id="{{ $option->id }}">
