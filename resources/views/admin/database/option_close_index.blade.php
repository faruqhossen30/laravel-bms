...
<h6 class="m-0 font-weight-bold text-primary">Close Options</h6>
</div>
<div class="filter-form p-3">
<form action="{{ url('tib-admin/database/close-options')}}" method="get">
<div class="input-group">
<input type="date" class="form-control" name="date" value="{{ request()->get('date') ? request()->get('date') : '' }}" placeholder="Date">
<input type="text" class="form-control" name="option" value="{{ request()->get('option') ? request()->get('option') : '' }}" placeholder="Option">
...
<td>
<a href="javascript:void(0)" id="option-to-live" data-id="{{ $row->id }}">
<button type="button" class="btn btn-primary btn-sm">
<i class="fas fa-calendar-check"></i> To Live
</button>
</a>
<a href="javascript:void(0)" id="option-to-upcoming" data-id="{{ $row->id }}">
